...
const ECHOES = [
  ...
  {
    path: '/audios/echoes/winning.mp3',
    name: 'Bravo',
    emoji: 'confetti_ball',
    isConfetti: 'confetti',
  },
...
export function ReactWithEmojiToggle() {
  const { isHost, playConfetti } = useEventSession()
  ...
  const handleSound = (soundIndex: number) => {
    ...
    flyEmoji({
      emoji: ECHOES[soundIndex - 1].emoji,
      name: `${ECHOES[soundIndex - 1].name} played by ${selfParticipant.name}`,
      participantId: selfParticipant.id,
    })

    if (ECHOES[soundIndex - 1].isConfetti) {
      playConfetti()
    }
  }
  ...
  useHotkeys('e+7', () => handleSound(7), liveHotKeyProps)
