  ...
  const libraryQuery = useQuery({
    ...
    queryFn: () =>
      LibraryService.getFrameFromLibrary({
        ...
        page,
        frameTypes:
          frameType !== AllFrameTypes[0].title ? [frameType] : frameTypes,
      }),
                ...
                {libraryQuery.data?.data?.map((libraryItem: any) => (
                  <div
                    ...
                    <div className="flex justify-between items-center py-1">
                      <p
                        className={cn(
                          'text-gray-600 text-xs font-medium text-ellipsis max-w-32 whitespace-nowrap overflow-hidden',
                          {
                            'text-gray-500': !libraryItem.frame?.name,
                          }
                        )}>
                        {libraryItem.frame?.name || 'Unnamed'}
                      </p>
                      {allowFrameDelete ? (
                        <DropdownActions
                          triggerIcon={
