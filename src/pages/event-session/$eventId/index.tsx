...
function EventSessionPage() {
  ...
  useEffect(() => {
    if (
      ...
      !userPreferences?.meeting?.layout
    ) {
      return
    }
    ...
    dispatch(
      changeContentTilesLayoutConfigAction({
        maxTilesPerPage: userPreferences.meeting.maxTilesPerPage ?? 6,
        layout: userPreferences?.meeting?.layout ?? ContentTilesLayout.Default,
      })
    )
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userPreferences.meeting.maxTilesPerPage, userPreferences.meeting.layout])
