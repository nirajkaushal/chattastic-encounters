...
export function MeetingSetupScreen() {
  ...
  useEffect(() => {
    ...
    meeting.self.on('deviceUpdate', (payload) => {
      if (payload.device.kind === 'audioinput') {
        updateAudioDeviceIdPreferences(payload.device.deviceId)
      } else if (payload.device.kind === 'audiooutput') {
        updateAudioOutputDeviceIdPreferences(payload.device.deviceId)
      } else if (payload.device.kind === 'videoinput') {
        updateVideoDeviceIdPreferences(payload.device.deviceId)
      }
    })
  ...
  useEffect(() => {
    const fetchAndSetDevices = async () => {
      const audioDevices = await selfParticipant.getAudioDevices()
      const audioOutputDevices = await selfParticipant.getSpeakerDevices()
      const videoDevices = await selfParticipant.getVideoDevices()
      const audioDevice = audioDevices.find(
        (device) => device.deviceId === userPreferences.meeting.audioDeviceId
      )
      const audioOutputDevice = audioOutputDevices.find(
        (device) =>
          device.deviceId === userPreferences.meeting.audioOutputDeviceId
      )
      const videoDevice = videoDevices.find(
        (device) => device.deviceId === userPreferences.meeting.videoDeviceId
      )

      if (audioDevice) {
        selfParticipant.setDevice(audioDevice)
      }
      if (audioOutputDevice) {
        selfParticipant.setDevice(audioOutputDevice)
      }
      if (videoDevice) {
        selfParticipant.setDevice(videoDevice)
      }
    }
    ...
    userPreferences.meeting.audioDeviceId,
    userPreferences.meeting.videoDeviceId,
  ])
